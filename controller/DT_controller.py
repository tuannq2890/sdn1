...
class SimpleMonitor13(switch.SimpleSwitch13):
    ...
    def flow_predict(self):
        try:
            ...
            pre_flow = model.predict(predict_flow_dataset)
            ...
            df = pd.DataFrame(predict_flow_dataset)
            ...
            if (count_1):
                list1 = df.loc[np.where(label_value==1), 5]
                host1 = np.array(list1)
                self.logger.info("-->Victim: {}".format(np.unique(host1)))

            count_2 = (label_value == 2).sum()
            self.logger.info("Probality of TCP-SYN DDOS traffic: {}".format(count_2/pre_flow.shape[0]))
            if (count_2):
                list2 = df.loc[np.where(label_value==2), 5]
                host2 = np.array(list2)
                self.logger.info("-->Victim: {}".format(np.unique(host2)))

            count_3 = (label_value == 3).sum()
            self.logger.info("Probality of UDP DDOS traffic: {}".format(count_3/pre_flow.shape[0]))
            if (count_3):
                list3 = df.loc[np.where(label_value==3), 5]
                host3 = np.array(list3)
                self.logger.info("-->Victim: {}".format(np.unique(host3)))
